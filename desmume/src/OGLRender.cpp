...
//				X432R::ShowDebugMessage("OpenGL FogAlphaOnly");
	...
	Render3DError OpenGLRenderer_X432::DoRender(const GFX3D_State *renderState, const VERTLIST *vertList, const POLYLIST *polyList, const INDEXLIST *indexList)
	{
		...
		#ifdef X432R_OPENGL_CUSTOMSTENCILTEST
		...
		for(u32 i = 0; i < polyCount; ++i)
		{
			...
			if( (poly_id == 0) || ( std::find( shadowPolygonIDs.begin(), shadowPolygonIDs.end(), poly_id ) != shadowPolygonIDs.end() ) ) continue;
			
			shadowPolygonIDs.push_back(poly_id);
		}
		
		#ifdef X432R_CUSTOMRENDERER_DEBUG
		if( shadowPolygonIDs.size() > 0 )
			ShowDebugMessage( "ShadowPolygon:" + std::to_string( shadowPolygonIDs.size() ) );
		#endif
		#endif
...
//		static const GLenum oglDepthFunc[2] = {GL_LESS, GL_EQUAL};
//		glDepthFunc(oglDepthFunc[attr.enableDepthTest]);
		#ifndef X432R_OPENGL_DEPTHTESTHACK
		glDepthFunc(attr.enableDepthTest ? GL_EQUAL : GL_LESS);
		#else
		glDepthFunc(attr.enableDepthTest ? GL_EQUAL : GL_LEQUAL);		// temp, ìp èB|SÀWYððÁµÈ¢Æª{IÈðÉÍÈçÈ¢
		#endif
		...
		#else
		// memo: SoftRastÅÍ3ÂÌXeVobt@ªgp³êÄ¢é
		// 
		// Fragment.polyid.opaque
		// úl: (renderState->clearColor >> 24) & 0x3F
		// s§¾|S`æÉpolyIDðZbg
		// 
		// Fragment.polyid.translucent
		// úl: 0xFF
		// ¼§¾|S`æÉpolyIDðZbg, ¯êIDÈç`æðskip
		// 
		// Fragment.stencil
		// úl: 0
		// shadowp
		// (ID == 0) `æÉ+1
		// (ID != 0) `æÉ-1 (½¾µFragment.polyid.opaqueÉ¯êIDªZbg³êÄ¢éêÍ`æðskip)
		
		// OpenGLpFXeVobt@8bit
		// 0011 1111	bit0-5: polygonID 0`3F				func:GL_ALWAYS		ref:polygonID				mask:0xFF
		// 0100 0000	bit6: translucenttO				func:GL_NOTEQUAL	ref:(polygonID | 0x40)		mask:0x7F
		// 1000 0000	bit7: shadowtO		(ID == 0)	func:				ref:						mask:
		//										(ID != 0)	func:				ref:						mask:
		
		#if 1
		if(attr.polygonMode == 3)		// polygonMode 0:modulate 1:decal 3:toon/highlight(gfx3d.renderState.shadingÅtoon/highlightIð) 3:shadow
		{
			xglEnable(GL_STENCIL_TEST);
			...
			if(attr.polygonID == 0)
			{
				...
				glStencilOp(GL_KEEP, GL_ZERO, GL_KEEP);						// fvXeXgfailÌÝXeVlÉ0ðZbg(clearStencilÍ0xFFÅè)
				glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
...
//				glStencilOp(GL_KEEP, GL_KEEP, GL_INVERT);					// XeVlð½](0 ¨ 0xFF)
				glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);
				glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
				glDepthMask(GL_TRUE);
			}
...
//		else if(attr.isTranslucent)
		else if( attr.isTranslucent && ( (attr.polygonMode != 1) || !attr.isOpaque ) )		// modeªdecalÌêÍeNX`ÌAt@lª³³êé½ß(isTranslucent && attr.isOpaque)ÈçisTranslucentð³
		{
			#if 1
			xglEnable(GL_STENCIL_TEST);
			glStencilFunc(GL_NOTEQUAL, attr.polygonID | 0x40, 0xFF);		// ¯êIDO[vÌ¼§¾|Sð2x«µÈ¢æ¤É·é
			glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
			#else
		...
		else if( std::find( shadowPolygonIDs.begin(), shadowPolygonIDs.end(), attr.polygonID ) != shadowPolygonIDs.end() )
		{
			// shadow|SÆ¯êIDO[vÌêÍXeVlÉ0x80ðZbgishadow|Sª¡ éêA³íÉ`æ³êÈ¢Â\« èj
			xglEnable(GL_STENCIL_TEST);
...
//			glStencilOp(GL_REPLACE, GL_REPLACE, GL_REPLACE);
		...
		else
		{
			#if 0
			xglEnable(GL_STENCIL_TEST);
			glStencilFunc(GL_ALWAYS, attr.polygonID, 0xFF);
//			glStencilOp(GL_REPLACE, GL_REPLACE, GL_REPLACE);
			glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
			#else
			xglDisable(GL_STENCIL_TEST);
...
//			glStencilOp(GL_REPLACE, GL_REPLACE, GL_REPLACE);
			...
			if( !attr.isTranslucent || ( (attr.polygonMode == 1) && attr.isOpaque ) || attr.enableAlphaDepthWrite )
				glDepthMask(GL_TRUE);	// modeªdecalÌêÍeNX`ÌAt@lª³³êé½ß(isTranslucent && attr.isOpaque)ÈçenableAlphaDepthWriteð³
			else
				glDepthMask(GL_FALSE);
		}
		#endif
		
/*		// (clearcolor.A == 0)ÌêÅàclearcolorÆfragmentcolorª¬³êÄµÜ¤½ß\¦Fª¨©µ­Èé»Ûðñð
...
//				downscaled_index = downscaled_y + (x / RENDER_MAGNIFICATION);
				...
				*highreso_buffer = color_rgba8888.Color;
				...
				// Ï¥NearestNeighboriS_ª§¾¾Á½êA¶ãAEãA¶ºAEºÌÅs§¾Ì_ðÌp·éj
				if( (RENDER_MAGNIFICATION == 2) || ( remainder_y != (1 << 16) ) || (remainder_x != 1) )
				{
					switch(remainder_y | remainder_x)
					{
						case 0:												// remainder_x == 0, remainder_y == 0
							*gfx3d_buffer = color_rgba8888.Color;			// gfx3d_ConvertedScreenàÌsNZtH[}bgÍRGBA6665¾ªAêIÉRGBA8888ÌÜÜlðÛ¶
							break;
...
//								color_rgba8888.A = 0xFF;
							...
							*gfx3d_buffer = BGRA8888_32_To_RGBA6665_32(color_rgba8888.Color);
							#else
							*gfx3d_buffer = BGRA8888_32Rev_To_RGBA6665_32Rev(color_rgba8888.Color);	// sNZtH[}bgðRGBA6665ÉÏ·µÄlðÛ¶
							#endif
							break;
				...
				if(color_rgba8888.A > 0)		// (remainder_x == 1, remainder_y == 1)FÈÕS_»èix2:2æfÌE¤, x3:3æfÌ, x4:4æfÌ¶©ç2ÂßðS_Æ©È·j
					*gfx3d_buffer = color_rgba8888.Color;
...
/*				#if 1
				...
				if( (remainder_x == 1) && (remainder_y == 1) && (color_rgba8888.A > 0) )	// ÈÕS_»èix2:2æfÌE¤, x3:3æfÌ, x4:4æfÌ¶©ç2ÂßðS_Æ©È·j
					*gfx3d_buffer = color_rgba8888.Color;
				...
				#else
				// ÈÕNearestNeighborix2:2æfÌE¤, x3:3æfÌ, x4:4æfÌ¶©ç2ÂßðS_Æ©È·j
