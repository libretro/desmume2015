...
public:
	...
	struct Sampler
	{
		...
		FORCEINLINE void clamp(s32 &val, const int size, const int sizemask){
			if(val<0) val = 0;
		...
		FORCEINLINE void flip(s32 &val, const int size, const int sizemask) {
			val &= ((size<<1)-1);
		...
		FORCEINLINE void dowrap(s32& iu, int& iv)
		{
			switch(wrap) {
				//flip none
