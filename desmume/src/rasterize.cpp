...
void SoftRasterizerEngine::ProcessClippedPolygons(const u32 width, const u32 height)
{
	...
	for(i = 0; i < clippedPolyCounter; ++i)			// [vð1ÂÉÜÆßÄ¬»ÌÂàè
	{
...
void SoftRasterizerEngine::DrawFog_forHighResolution()
{
	...
	for(u32 i = 0; i < fragment_count; ++i)
	{
		...
		else
		{
			...
			// tHOÌ`æÊðWSoftRasterizerÆ¯¶É·é½ßÉtHOÌalphað³
			if(FOG_ALPHA_ONLY)
				destFragmentColor->a = ( (fog2 * destFragmentColor->a) + (a * fog) ) >> 7;
			else
			{
...
//		mainSoftRasterizer.ProcessClippedPolygons<RENDER_MAGNIFICATION>();
	...
	static inline RGBA8888 FragmentColorToRGBA8888(const FragmentColor color)
	{
		...
		if(color.a == 0)
			return result;		// RGBA8888Í¶¬É0Åú»³êÄ¢é
		
		// RGBA6665 ¨ BGRA8888
		result.R = color.r << 2;
		result.G = color.g << 2;
		result.B = color.b << 2;
//		result.A = (color.a << 3) + 7;	// 0x1Fð3rbg¶VtgµÄàs§¾ÉÈçÈ¢½ßlðItZbg
		result.A = color.a << 3;
...
//								color_rgba8888.A = 0xFF;
							...
							*gfx3d_buffer = RGBA8888ToFragmentColor(color_rgba8888).color;			// sNZtH[}bgðRGBA6665ÉÏ·µÄlðÛ¶
							break;
				...
				if(color_rgba8888.A > 0)		// (x == 1, y == 1)FÈÕS_»èix2:2æfÌE¤, x3:3æfÌ, x4:4æfÌ¶©ç2ÂßðS_Æ©È·j
					*gfx3d_buffer = color_rgba8888.Color;
