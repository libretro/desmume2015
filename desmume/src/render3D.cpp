...
Render3DError Render3D::FlushFramebuffer(FragmentColor *__restrict dstRGBA6665, u16 *__restrict dstRGBA5551)
{
	...
	for (size_t i = 0; i < (this->_framebufferWidth * this->_framebufferHeight); i++)
	{
		dstRGBA5551[i] = R6G6B6TORGB15(
            ...
            this->_framebufferColor[i].g,
            this->_framebufferColor[i].b) | ((this->_framebufferColor[i].a == 0) ? 0x0000 : 0x8000);
	}
...
Render3DError Render3D::ClearFramebuffer(const GFX3D_State &renderState)
{
	...
	if (renderState.enableClearImage)
	{
		...
		error = this->ClearUsingImage(
            this->clearImageColor16Buffer,
            this->clearImageDepthBuffer,
            this->clearImageFogBuffer,
            this->clearImagePolyIDBuffer);
		if (error != RENDER3DERROR_NOERR)
...
Render3DError Render3D::ClearUsingImage(
      const u16 *__restrict colorBuffer,
      const u32 *__restrict depthBuffer,
      const u8 *__restrict fogBuffer,
      const u8 *__restrict polyIDBuffer)
{
	return RENDER3DERROR_NOERR;
}

Render3DError Render3D::ClearUsingValues(
      const FragmentColor &clearColor,
      const FragmentAttributes &clearAttributes) const
{
	return RENDER3DERROR_NOERR;
}
...
Render3DError Render3D::Render(const GFX3D &engine)
{
	...
	if (engine.renderState.enableEdgeMarking)
		this->RenderEdgeMarking(engine.renderState.edgeMarkColorTable,
            engine.renderState.enableAntialiasing);
	
	if (engine.renderState.enableFog)
	{
		this->RenderFog(
            engine.renderState.fogDensityTable,
            engine.renderState.fogColor,
            engine.renderState.fogOffset,
            engine.renderState.fogShift,
            engine.renderState.enableFogAlphaOnly);
	}
